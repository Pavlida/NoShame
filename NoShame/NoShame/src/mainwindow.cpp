    ...
    , ui(new Ui::MainWindow)
{
    ...
    nutriens = new Nutriens(this);
    nutriens->setWindowTitle("Nutriens");
...
void MainWindow::connectTableSignals() const
{
    ...
    connect(rcTable, SIGNAL(giveDetails(QModelIndex)), this, SLOT(viewItemNutriens(QModelIndex)));
}
...
void MainWindow::on_actionLoad_another_menu_triggered()
{
    ...
    try
    {
        ...
        nutriens->setHeaders(McMenu->getHeaders()); //give the nutrien window headers for constructing  properly
    }
    ...
    else
    {
        ...
        else
        {
            ...
            connect(Qty, QOverload<int>::of(&QSpinBox::valueChanged), [this, calories, defaultCalories, index](int value) //change the calorie number if user adds more of the same item
            {
                ...
                nutriens->changeQuantity(index, value); //change the Q-ty for counting nutriens
            });
            ...
            connect(deleteItem, &QToolButton::clicked, [this, calories, frame, itemName]()
            {
                ...
                nutriens->deleteMealItem(ui->MealLayout->indexOf(frame) - 1); // actual number of element in meal
            ...
            nutriens->addMealItem(McMenu->getItem(userMenu->mapToSource(i).row()));
        }
    }
}
...
void MainWindow::viewItemNutriens(QModelIndex i) const
{
    nutriens->displayItem(McMenu->getItem(userMenu->mapToSource(i).row()));
    nutriens->show();
}
